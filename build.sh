#!/usr/bin/env bash
#
# build.sh - Assemble the Julian Date Java Calc add-in into build/JulianDate.oxt
#
# Pipeline:
#   1. unoidl-write : idl/**.idl                  -> build/types/XJulianDate.rdb  (UNO type library)
#   2. javamaker    : build/types/XJulianDate.rdb -> build/gen/**.class           (Java bindings)
#   3. javac        : src/**.java + bindings      -> build/classes/**.class
#   4. jar          : classes + bindings          -> build/oxt/julian-date.jar    (+ RegistrationClassName)
...
#     the JRE LibreOffice accepts; see docs/INSTALL.md.
#
# Usage:
#   ./build.sh
...
echo '[1/5] unoidl-write : idl -> build/types/XJulianDate.rdb'
rdb="$type_dir/XJulianDate.rdb"
"$uw" "$types" "$root/idl" "$rdb"

# --- 2. Generate Java bindings from the type library ------------------------
echo '[2/5] javamaker    : build/types/XJulianDate.rdb -> build/gen'
...
echo '[4/5] jar          : classes + bindings -> build/oxt/julian-date.jar'
addin_jar="$stage/julian-date.jar"
mf="$root/registration/MANIFEST.MF"
# Merge classes + bindings into one tree first: this JDK's `jar` tool rejects
# duplicate directory entries (e.g. com/example/juliandate/) when given as
# two separate -C trees, even though the class files themselves don't collide.
jar_dir="$build/jarstage"
rm -rf "$jar_dir"
mkdir -p "$jar_dir"
cp -r "$cls_dir"/. "$jar_dir"/
cp -r "$gen_dir"/. "$jar_dir"/
"$jar" cfm "$addin_jar" "$mf" -C "$jar_dir" .

# --- 5. Stage remaining package files and zip the .oxt ----------------------
echo '[5/5] zip          : staging -> build/JulianDate.oxt'
cp "$rdb" "$stage/types/XJulianDate.rdb"
...
oxt="$build/JulianDate.oxt"
rm -f "$oxt"
( cd "$stage" && zip -q -r "$oxt" . )

echo
echo "Built $oxt"
echo 'Install with:  unopkg add --force build/JulianDate.oxt   (see docs/INSTALL.md)'
