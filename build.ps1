<#
    build.ps1 - Assemble the Julian Date Java Calc add-in into build/JulianDate.oxt
    ...
    Requirements:
      ...
      - A JDK (any 8+) to compile with. Resolved from -Jdk, else JAVA_HOME, else
        ...
        JRE LibreOffice accepts; see docs/INSTALL.md.
...
Write-Host '[1/5] unoidl-write : idl -> build/types/XJulianDate.rdb'
$rdb = Join-Path $typeDir 'XJulianDate.rdb'
...
Write-Host '[2/5] javamaker    : build/types/XJulianDate.rdb -> build/gen'
...
Write-Host '[4/5] jar          : classes + bindings -> build/oxt/julian-date.jar'
$addinJar = Join-Path $stage 'julian-date.jar'
$mf = Join-Path $root 'registration\MANIFEST.MF'
& $jar 'cfm' $addinJar $mf '-C' $clsDir '.' '-C' $genDir '.'
if ($LASTEXITCODE -ne 0) { throw "jar failed (exit $LASTEXITCODE)" }

# --- 5. Stage remaining package files and zip the .oxt --------------------
Write-Host '[5/5] zip           : staging -> build/JulianDate.oxt'
Copy-Item $rdb                                              (Join-Path $stage 'types\XJulianDate.rdb')
...
$oxt = Join-Path $build 'JulianDate.oxt'
...
Write-Host 'Install with:  unopkg add --force build\JulianDate.oxt   (see docs/INSTALL.md)'
